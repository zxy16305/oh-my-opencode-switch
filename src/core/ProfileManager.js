...
import { ProfileError, ConfigError } from '../utils/errors.js';
import { ConfigManager } from './ConfigManager.js';
import { ProfileRenderer, DEFAULT_TEMPLATE_JSON } from './ProfileRenderer.js';
...
export class ProfileManager {
  ...
  async _switchTemplateProfile(name) {
    ...
    const renderedConfig = await this.renderer.renderTemplate(templateObj, variables);
    await this.configManager.writeConfig(renderedConfig);
  }

  /**
   * Process model arrays in config (delegates to ProfileRenderer)
  ...
  _processModelArrays(config) {
    return this.renderer._processModelArrays(config);
  }
